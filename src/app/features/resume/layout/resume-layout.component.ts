...
export class ResumeLayoutComponent {
  ...
  // SEC-F-06: bypassSecurityTrustHtml is used below for SVG icon strings. This is acceptable because:
  // 1. All SVG strings are static, hardcoded constants defined in source code — no user input.
  // 2. Angular's built-in sanitizer strips SVG elements from [innerHTML] bindings, making
  //    DomSanitizer.sanitize(SecurityContext.HTML) insufficient for SVG rendering.
  // 3. Inline SVG in the template is not feasible here because the icons are data-driven
  //    (computed from i18n labels) and rendered via *ngFor on the menuItems array.
  // Alternative considered: Using a SafeIconPipe — but the pipe would still call
  // bypassSecurityTrustHtml internally, adding indirection without improving security.
  menuItems = computed(() => [
    {
